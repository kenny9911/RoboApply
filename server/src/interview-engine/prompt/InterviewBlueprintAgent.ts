...
- Every "q" is SPOKEN ALOUD to the candidate by a text-to-speech voice, so write each one as plain conversational prose a person could say out loud: no markdown, no backticks, no code blocks, no bullet lists, no emoji, no URLs. For a technical or coding question, describe the signature, the types and the data shape IN WORDS — say "a function called mergeHistory that takes the snapshot and the deltas and returns a list of items, where each item has an id, an updatedAt timestamp, a title and a status" rather than pasting a type literal. The candidate reads the same text on screen, so it must also stand on its own without formatting.
