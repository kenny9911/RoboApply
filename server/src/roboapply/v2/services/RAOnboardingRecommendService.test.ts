...
import { evaluateCachedScore, passesPrefilter } from './RAOnboardingRecommendService.js';

describe('evaluateCachedScore — scorer provenance', () => {
  const currentModel = 'openrouter/openai/gpt-5.6-luna';
  const freshExplanation = { responseLanguage: 'en', promptVersion: 'score_v1' };

  it('reuses score and prose only when hash, model, locale, and prompt match', () => {
    expect(evaluateCachedScore(
      {
      ...
      'en',
      currentModel,
    )).toEqual({ fresh: true, scoreOnly: false });
  });

  it('fully invalidates a score generated by a different or unknown model', () => {
    for (const modelUsed of ['anthropic/claude-sonnet-4.6', null]) {
      expect(evaluateCachedScore(
        {
          resumeContentHashAtScore: 'resume-hash',
          modelUsed,
        ...
        'en',
        currentModel,
      )).toEqual({ fresh: false, scoreOnly: false });
    }
  });

  it('keeps only the numeric score reusable for same-model stale prose', () => {
    expect(evaluateCachedScore(
      {
        resumeContentHashAtScore: 'resume-hash',
        modelUsed: currentModel,
        explanation: { responseLanguage: 'zh', promptVersion: 'score_v1' },
      },
      'resume-hash',
      'en',
      currentModel,
    )).toEqual({ fresh: false, scoreOnly: true });
  });
});
