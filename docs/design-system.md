...
- **Interview prep:** one screen, not a wizard. Past practice → the job (the only required choice) → a pre-matched plan shown as editable chips with inline option trays → a launch dock that is live from the moment a job is chosen. A step sequence is wrong here: the recommendation engine fills every field but the job, so steps after the first ask nothing. During a live interview the screen is shaped like a call — the interviewer holds the stage, the candidate's own tile is pinned into it rather than hidden behind a disclosure, coach and transcript share an on-demand rail, and mic / camera / end sit in a bottom control bar. Reports state the score once, lead with the single next action, and put recommendations, per-question review, recording and transcript behind one tab strip rather than stacked disclosures.
