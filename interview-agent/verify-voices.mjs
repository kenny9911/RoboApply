...
// commits. A runtime fallback can also be unavailable, so validate the primary
// itself. This probe synthesizes a short phrase through EACH catalog voice
...
const MALE = { model: 'cartesia/sonic-3', voiceId: 'a167e0f3-df7e-4d52-a9c3-f949145efdab' };
const VOICES = [
  ...
  { locale: 'zh-TW', g: 'female', model: 'cartesia/sonic-3',             voiceId: 'e90c6678-f0d3-4767-9883-5d0ecf5894a8', lang: 'zh', text: '你好，謝謝你今天來參加面試。' },
...
// Exercise the male voice in every supported language, including Traditional Chinese.
VOICES.push(...VOICES.map((v) => ({ ...v, g: 'male', ...MALE })));
...
async function probe(v) {
  ...
  const seconds = (samples / engine.sampleRate).toFixed(1);
...
async function main() {
  ...
  console.log('   Set both INTERVIEW_ENGINE_MODEL_<LOCALE>[_MALE] and');
  console.log('   INTERVIEW_ENGINE_VOICE_<LOCALE>[_MALE] when overriding the provider.');
  console.log('──────────────────────────────────────\n');
